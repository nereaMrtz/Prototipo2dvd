    ...
    public class SplitScreenEffect : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        void LateUpdate()
        {
            ...
            switch (Screens.Count)
            {
                ...
                case 2:
                    ...
                    foreach (var component in m_Components)
                    {
                        if (component is ISplitScreenVoronoi modifier && component.isActiveAndEnabled)
                        {
                            modifier.OnSplitScreenVoronoi(ref m_SplitScreen2.VoronoiBuilder, ref m_SplitScreen2.VoronoiDiagram, ref m_SplitScreen2.Sites);
                        }
                    }
                ...
                case 3:
                    ...
                    foreach (var component in m_Components)
                    {
                        if (component is ISplitScreenVoronoi modifier && component.isActiveAndEnabled)
                        {
                            modifier.OnSplitScreenVoronoi(ref m_SplitScreen3.VoronoiBuilder, ref m_SplitScreen3.VoronoiDiagram, ref m_SplitScreen3.Sites);
                        }
                    }
                ...
                case 4:
                    ...
                    foreach (var component in m_Components)
                    {
                        if (component is ISplitScreenVoronoi modifier && component.isActiveAndEnabled)
                        {
                            modifier.OnSplitScreenVoronoi(ref m_SplitScreen4.VoronoiBuilder, ref m_SplitScreen4.VoronoiDiagram, ref m_SplitScreen4.Sites);
                        }
                    }
        ...
        void UpdatePlaneMatrices()
        {
            ...
            if (!UseAspectRatio)
            {
                ...
                {
                    float4 ddd = math.mul(camera.projectionMatrix, new float4(0, 0, -Distance, 1));
                    float d = ddd.z / ddd.w;

                    float4 ss0 = new float4(-1, -1, d, 1);
                    float4 ss1 = new float4(1, 1, d, 1);

                    float4 ww0 = math.mul(math.inverse(camera.projectionMatrix), ss0);
                    float4 ww1 = math.mul(math.inverse(camera.projectionMatrix), ss1);

                    ww0 /= ww0.w;
                    ww1 /= ww1.w;

                    float4x4 orth = float4x4.OrthoOffCenter(ww0.x, ww1.x, ww0.y, ww1.y, camera.nearClipPlane, camera.farClipPlane);
